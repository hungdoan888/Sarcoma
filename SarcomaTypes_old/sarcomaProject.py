...
def clinicalData(nameOfSarcoma, myFile):
    ...
    numberUnknownEthnicity = df_ethnicity[df_ethnicity["Ethnicity Category"] == "Unknown"]["Patient ID"].count()
    numberNotCollectedEthnicity = df_ethnicity[df_ethnicity["Ethnicity Category"] == "Not Collected"]["Patient ID"].count()
    numberNotcollectedEthnicity = df_ethnicity[df_ethnicity["Ethnicity Category"] == "Not collected"]["Patient ID"].count()
    ...
    numberOfRaceUnknown = df_race[df_race["Primary Race"] == "Unknown"]["Patient ID"].count()
    numberOfRaceNotCollected = df_race[df_race["Primary Race"] == "Not Collected"]["Patient ID"].count()
    numberOfRaceNotcollected = df_race[df_race["Primary Race"] == "Not collected"]["Patient ID"].count()
    ...
    df_cd_out = pd.DataFrame({"Sarcoma Name From File": [nameOfSarcoma],
                              ...
                              "Unknown (Ethnicity)": [numberUnknownEthnicity],
                              "Not Collected (Ethnicity)": [numberNotCollectedEthnicity + numberNotcollectedEthnicity],
                              "White": [numberOfWhite],
                              "Black": [numberOfBlack], 
                              "Asian": [numberOfAsian], 
                              ...
                              "Unknown (Race)": [numberOfRaceUnknown],
                              "Not Collected (Race)": [numberOfRaceNotCollected + numberOfRaceNotcollected],
...
def mutatedGenes(nameOfSarcoma, nameOfSarcomaDict, myFile):
    ...
    df_mg_out = df_mg[["Sarcoma Name From File", "Cancer Type Detailed", "Gene", "#"]].sort_values("#", ascending=False)
    return df_mg_out
...
def structuralVariantGenes(nameOfSarcoma, nameOfSarcomaDict, myFile):
    ...
    df_sv_out = df_sv[["Sarcoma Name From File", "Cancer Type Detailed", "Gene", "#"]].sort_values("#", ascending=False)
    return df_sv_out
...
def CNAGenes(nameOfSarcoma, nameOfSarcomaDict, myFile):
    ...
    df_cna_out = df_cna[["Sarcoma Name From File", "Cancer Type Detailed", "Gene", "CNA", "#"]].sort_values("#", ascending=False)
    return df_cna_out
...
if __name__ == "__main__":
    ...
    # Create Pivot Tables
    df_mg_pivot = pd.pivot_table(df_mg_master, values="#", index="Gene", columns="Cancer Type Detailed", aggfunc=np.sum, fill_value=0)
    df_sv_pivot = pd.pivot_table(df_sv_master, values="#", index="Gene", columns="Cancer Type Detailed", aggfunc=np.sum, fill_value=0)
    df_cna_pivot = pd.pivot_table(df_cna_master, values="#", index="Gene", columns="Cancer Type Detailed", aggfunc=np.sum, fill_value=0)
    
    # Add Totals
    df_mg_pivot.loc["Total", :] = df_mg_pivot.sum(axis=0)
    df_mg_pivot.loc[:, "Total"] = df_mg_pivot.sum(axis=1)
    df_sv_pivot.loc["Total", :] = df_sv_pivot.sum(axis=0)
    df_sv_pivot.loc[:, "Total"] = df_sv_pivot.sum(axis=1)
    df_cna_pivot.loc["Total", :] = df_cna_pivot.sum(axis=0)
    df_cna_pivot.loc[:, "Total"] = df_cna_pivot.sum(axis=1)
    ...
    with pd.ExcelWriter(output) as writer:
        ...
        df_mg_pivot.to_excel(writer, sheet_name="Mutated Genes Pivot", index=True)
        df_sv_master.to_excel(writer, sheet_name="Structural Variant Genes", index=False)
        df_sv_pivot.to_excel(writer, sheet_name="Structural Variant Genes Pivot", index=True)
        df_cna_master.to_excel(writer, sheet_name="CNA Genes", index=False)
        df_cna_pivot.to_excel(writer, sheet_name="CNA Genes Pivot", index=True)
